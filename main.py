...
def find_all_matches(block, full_data, used_indices):
    ...
    for i in reversed(range(len(full_data) - block_len)):
        index_range = set(range(i, i + block_len))
        if index_range & used_indices:
            continue

        candidate = full_data[i:i + block_len]
        if candidate == block:
            used_indices.update(index_range)
...
def predict():
    try:
        ...
        selected_mode = mode.split("_")[1]
        flow = transform_modes[selected_mode](recent_flow)
        top, bottom = find_all_matches(flow, all_data, used_indices)
